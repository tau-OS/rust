...
pub trait DesktopExt: IsA<Desktop> + sealed::Sealed + 'static {
    ...
    fn connect_scheme_variant_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_scheme_variant_trampoline<P: IsA<Desktop>, F: Fn(&P) + 'static>(this: *mut ffi::HeDesktop, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(Desktop::from_glib_borrow(this).unsafe_cast_ref())
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"notify::scheme-variant\0".as_ptr() as *const _,
                Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(notify_scheme_variant_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
