...
glib::wrapper! {
    ...
    pub struct SettingsWindow(Object<ffi::HeSettingsWindow, ffi::HeSettingsWindowClass>) @extends Window, gtk::Window, gtk::Widget, gobject::InitiallyUnowned, @implements gtk::Accessible, gtk::Buildable, gtk::ConstraintTarget, gtk::Native, gtk::Root, gtk::ShortcutManager;

    match fn {
        type_ => || ffi::he_settings_window_get_type(),
    }
}

impl SettingsWindow {
        pub const NONE: Option<&'static SettingsWindow> = None;
    

    #[doc(alias = "he_settings_window_new")]
    pub fn new(parent: Option<&impl IsA<gtk::Window>>) -> SettingsWindow {
        assert_initialized_main_thread!();
        unsafe {
            from_glib_none(ffi::he_settings_window_new(parent.map(|p| p.as_ref()).to_glib_none().0))
        }
    }

            // rustdoc-stripper-ignore-next
            /// Creates a new builder-pattern struct instance to construct [`SettingsWindow`] objects.
            ///
            /// This method returns an instance of [`SettingsWindowBuilder`](crate::builders::SettingsWindowBuilder) which can be used to create [`SettingsWindow`] objects.
            pub fn builder() -> SettingsWindowBuilder {
                SettingsWindowBuilder::default()
            }
...
// rustdoc-stripper-ignore-next
        /// A [builder-pattern] type to construct [`SettingsWindow`] objects.
        ///
...
impl SettingsWindowBuilder {
    // rustdoc-stripper-ignore-next
    /// Create a new [`SettingsWindowBuilder`].
    pub fn new() -> Self {
        Self::default()
    }
...
if let Some(ref width_request) = self.width_request {
        ...
        glib::Object::new::<SettingsWindow>(&properties)
...
impl<O: IsA<SettingsWindow>> SettingsWindowExt for O {
    fn add_page(&self, page: &impl IsA<SettingsPage>) {
        unsafe {
            ffi::he_settings_window_add_page(self.as_ref().to_glib_none().0, page.as_ref().to_glib_none().0);
        }
    }

    fn add_list(&self, list: &impl IsA<ContentList>) {
        unsafe {
            ffi::he_settings_window_add_list(self.as_ref().to_glib_none().0, list.as_ref().to_glib_none().0);
        }
    }
}
