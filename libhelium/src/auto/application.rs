...
pub trait HeApplicationExt: IsA<Application> + sealed::Sealed + 'static {
    ...
    fn connect_scheme_factory_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_scheme_factory_trampoline<P: IsA<Application>, F: Fn(&P) + 'static>(this: *mut ffi::HeApplication, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(Application::from_glib_borrow(this).unsafe_cast_ref())
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"notify::scheme-factory\0".as_ptr() as *const _,
                Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(notify_scheme_factory_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
