...
pub trait MiniContentBlockExt: IsA<MiniContentBlock> + sealed::Sealed + 'static {
    ...
    fn set_paintable(&self, value: &impl IsA<gdk::Paintable>) {
        unsafe {
            ffi::he_mini_content_block_set_paintable(self.as_ref().to_glib_none().0, value.as_ref().to_glib_none().0);
        }
    }
    ...
    fn connect_paintable_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_paintable_trampoline<P: IsA<MiniContentBlock>, F: Fn(&P) + 'static>(this: *mut ffi::HeMiniContentBlock, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(MiniContentBlock::from_glib_borrow(this).unsafe_cast_ref())
        }
        unsafe {
