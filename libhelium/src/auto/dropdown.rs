// This file was generated by gir (https://github.com/gtk-rs/gir)
// from 
// from gir-files (https://github.com/gtk-rs/gir-files.git)
// DO NOT EDIT

use crate::{ffi};
use glib::{prelude::*,signal::{connect_raw, SignalHandlerId},translate::*};
use std::{boxed::Box as Box_};

glib::wrapper! {
    #[doc(alias = "HeDropdown")]
    pub struct Dropdown(Object<ffi::HeDropdown, ffi::HeDropdownClass>) @extends gtk::Grid, gtk::Widget, @implements gtk::Accessible, gtk::Buildable, gtk::ConstraintTarget, gtk::Orientable;

    match fn {
        type_ => || ffi::he_dropdown_get_type(),
    }
}

impl Dropdown {
    #[doc(alias = "he_dropdown_new")]
    pub fn new() -> Dropdown {
        assert_initialized_main_thread!();
        unsafe {
            from_glib_none(ffi::he_dropdown_new())
        }
    }

            // rustdoc-stripper-ignore-next
            /// Creates a new builder-pattern struct instance to construct [`Dropdown`] objects.
            ///
            /// This method returns an instance of [`DropdownBuilder`](crate::builders::DropdownBuilder) which can be used to create [`Dropdown`] objects.
            pub fn builder() -> DropdownBuilder {
                DropdownBuilder::new()
            }
        

    #[doc(alias = "he_dropdown_append")]
    pub fn append(&self, text: &str) {
        unsafe {
            ffi::he_dropdown_append(self.to_glib_none().0, text.to_glib_none().0);
        }
    }

    #[doc(alias = "he_dropdown_get_active")]
    #[doc(alias = "get_active")]
    pub fn active(&self) -> Option<glib::GString> {
        unsafe {
            from_glib_full(ffi::he_dropdown_get_active(self.to_glib_none().0))
        }
    }

    #[doc(alias = "he_dropdown_insert")]
    pub fn insert(&self, position: i32, text: &str) {
        unsafe {
            ffi::he_dropdown_insert(self.to_glib_none().0, position, text.to_glib_none().0);
        }
    }

    #[doc(alias = "he_dropdown_prepend")]
    pub fn prepend(&self, text: &str) {
        unsafe {
            ffi::he_dropdown_prepend(self.to_glib_none().0, text.to_glib_none().0);
        }
    }

    #[doc(alias = "he_dropdown_remove")]
    pub fn remove(&self, position: i32) {
        unsafe {
            ffi::he_dropdown_remove(self.to_glib_none().0, position);
        }
    }

    #[doc(alias = "he_dropdown_remove_all")]
    pub fn remove_all(&self) {
        unsafe {
            ffi::he_dropdown_remove_all(self.to_glib_none().0);
        }
    }

    #[doc(alias = "he_dropdown_get_active_id")]
    #[doc(alias = "get_active_id")]
    pub fn active_id(&self) -> Option<glib::GString> {
        unsafe {
            from_glib_none(ffi::he_dropdown_get_active_id(self.to_glib_none().0))
        }
    }

    #[doc(alias = "he_dropdown_set_active_id")]
    pub fn set_active_id(&self, value: Option<&str>) {
        unsafe {
            ffi::he_dropdown_set_active_id(self.to_glib_none().0, value.to_glib_none().0);
        }
    }

    #[doc(alias = "he_dropdown_get_max_width_chars")]
    #[doc(alias = "get_max_width_chars")]
    pub fn max_width_chars(&self) -> i32 {
        unsafe {
            ffi::he_dropdown_get_max_width_chars(self.to_glib_none().0)
        }
    }

    #[doc(alias = "he_dropdown_set_max_width_chars")]
    pub fn set_max_width_chars(&self, value: i32) {
        unsafe {
            ffi::he_dropdown_set_max_width_chars(self.to_glib_none().0, value);
        }
    }

    #[doc(alias = "he_dropdown_get_ellipsize")]
    #[doc(alias = "get_ellipsize")]
    pub fn ellipsize(&self) -> pango::EllipsizeMode {
        unsafe {
            from_glib(ffi::he_dropdown_get_ellipsize(self.to_glib_none().0))
        }
    }

    #[doc(alias = "he_dropdown_set_ellipsize")]
    pub fn set_ellipsize(&self, value: pango::EllipsizeMode) {
        unsafe {
            ffi::he_dropdown_set_ellipsize(self.to_glib_none().0, value.into_glib());
        }
    }

    #[doc(alias = "he_dropdown_get_dropdown")]
    #[doc(alias = "get_dropdown")]
    pub fn dropdown(&self) -> gtk::DropDown {
        unsafe {
            from_glib_none(ffi::he_dropdown_get_dropdown(self.to_glib_none().0))
        }
    }

    #[doc(alias = "he_dropdown_set_dropdown")]
    pub fn set_dropdown(&self, value: &gtk::DropDown) {
        unsafe {
            ffi::he_dropdown_set_dropdown(self.to_glib_none().0, value.to_glib_none().0);
        }
    }

    #[doc(alias = "changed")]
    pub fn connect_changed<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn changed_trampoline<F: Fn(&Dropdown) + 'static>(this: *mut ffi::HeDropdown, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(&from_glib_borrow(this))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"changed\0".as_ptr() as *const _,
                Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(changed_trampoline::<F> as *const ())), Box_::into_raw(f))
        }
    }

    #[doc(alias = "active-id")]
    pub fn connect_active_id_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_active_id_trampoline<F: Fn(&Dropdown) + 'static>(this: *mut ffi::HeDropdown, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(&from_glib_borrow(this))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"notify::active-id\0".as_ptr() as *const _,
                Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(notify_active_id_trampoline::<F> as *const ())), Box_::into_raw(f))
        }
    }

    #[doc(alias = "max-width-chars")]
    pub fn connect_max_width_chars_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_max_width_chars_trampoline<F: Fn(&Dropdown) + 'static>(this: *mut ffi::HeDropdown, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(&from_glib_borrow(this))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"notify::max-width-chars\0".as_ptr() as *const _,
                Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(notify_max_width_chars_trampoline::<F> as *const ())), Box_::into_raw(f))
        }
    }

    #[doc(alias = "ellipsize")]
    pub fn connect_ellipsize_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_ellipsize_trampoline<F: Fn(&Dropdown) + 'static>(this: *mut ffi::HeDropdown, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(&from_glib_borrow(this))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"notify::ellipsize\0".as_ptr() as *const _,
                Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(notify_ellipsize_trampoline::<F> as *const ())), Box_::into_raw(f))
        }
    }

    #[doc(alias = "dropdown")]
    pub fn connect_dropdown_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_dropdown_trampoline<F: Fn(&Dropdown) + 'static>(this: *mut ffi::HeDropdown, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(&from_glib_borrow(this))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"notify::dropdown\0".as_ptr() as *const _,
                Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(notify_dropdown_trampoline::<F> as *const ())), Box_::into_raw(f))
        }
    }
}

impl Default for Dropdown {
                     fn default() -> Self {
                         Self::new()
                     }
                 }

// rustdoc-stripper-ignore-next
        /// A [builder-pattern] type to construct [`Dropdown`] objects.
        ///
        /// [builder-pattern]: https://doc.rust-lang.org/1.0.0/style/ownership/builders.html
#[must_use = "The builder must be built to be used"]
pub struct DropdownBuilder {
            builder: glib::object::ObjectBuilder<'static, Dropdown>,
        }

        impl DropdownBuilder {
        fn new() -> Self {
            Self { builder: glib::object::Object::builder() }
        }

                            pub fn active_id(self, active_id: impl Into<glib::GString>) -> Self {
                            Self { builder: self.builder.property("active-id", active_id.into()), }
                        }

                            pub fn max_width_chars(self, max_width_chars: i32) -> Self {
                            Self { builder: self.builder.property("max-width-chars", max_width_chars), }
                        }

                            pub fn ellipsize(self, ellipsize: pango::EllipsizeMode) -> Self {
                            Self { builder: self.builder.property("ellipsize", ellipsize), }
                        }

                            pub fn dropdown(self, dropdown: &gtk::DropDown) -> Self {
                            Self { builder: self.builder.property("dropdown", dropdown.clone()), }
                        }

                            pub fn baseline_row(self, baseline_row: i32) -> Self {
                            Self { builder: self.builder.property("baseline-row", baseline_row), }
                        }

                            pub fn column_homogeneous(self, column_homogeneous: bool) -> Self {
                            Self { builder: self.builder.property("column-homogeneous", column_homogeneous), }
                        }

                            pub fn column_spacing(self, column_spacing: i32) -> Self {
                            Self { builder: self.builder.property("column-spacing", column_spacing), }
                        }

                            pub fn row_homogeneous(self, row_homogeneous: bool) -> Self {
                            Self { builder: self.builder.property("row-homogeneous", row_homogeneous), }
                        }

                            pub fn row_spacing(self, row_spacing: i32) -> Self {
                            Self { builder: self.builder.property("row-spacing", row_spacing), }
                        }

                            pub fn can_focus(self, can_focus: bool) -> Self {
                            Self { builder: self.builder.property("can-focus", can_focus), }
                        }

                            pub fn can_target(self, can_target: bool) -> Self {
                            Self { builder: self.builder.property("can-target", can_target), }
                        }

                            pub fn css_classes(self, css_classes: impl Into<glib::StrV>) -> Self {
                            Self { builder: self.builder.property("css-classes", css_classes.into()), }
                        }

                            pub fn css_name(self, css_name: impl Into<glib::GString>) -> Self {
                            Self { builder: self.builder.property("css-name", css_name.into()), }
                        }

                            //pub fn cursor(self, cursor: /*Ignored*/&gdk::Cursor) -> Self {
                        //    Self { builder: self.builder.property("cursor", cursor), }
                        //}

                            pub fn focus_on_click(self, focus_on_click: bool) -> Self {
                            Self { builder: self.builder.property("focus-on-click", focus_on_click), }
                        }

                            pub fn focusable(self, focusable: bool) -> Self {
                            Self { builder: self.builder.property("focusable", focusable), }
                        }

                            pub fn halign(self, halign: gtk::Align) -> Self {
                            Self { builder: self.builder.property("halign", halign), }
                        }

                            pub fn has_tooltip(self, has_tooltip: bool) -> Self {
                            Self { builder: self.builder.property("has-tooltip", has_tooltip), }
                        }

                            pub fn height_request(self, height_request: i32) -> Self {
                            Self { builder: self.builder.property("height-request", height_request), }
                        }

                            pub fn hexpand(self, hexpand: bool) -> Self {
                            Self { builder: self.builder.property("hexpand", hexpand), }
                        }

                            pub fn hexpand_set(self, hexpand_set: bool) -> Self {
                            Self { builder: self.builder.property("hexpand-set", hexpand_set), }
                        }

                            //pub fn layout_manager(self, layout_manager: &impl IsA</*Ignored*/gtk::LayoutManager>) -> Self {
                        //    Self { builder: self.builder.property("layout-manager", layout_manager.clone().upcast()), }
                        //}

                            pub fn margin_bottom(self, margin_bottom: i32) -> Self {
                            Self { builder: self.builder.property("margin-bottom", margin_bottom), }
                        }

                            pub fn margin_end(self, margin_end: i32) -> Self {
                            Self { builder: self.builder.property("margin-end", margin_end), }
                        }

                            pub fn margin_start(self, margin_start: i32) -> Self {
                            Self { builder: self.builder.property("margin-start", margin_start), }
                        }

                            pub fn margin_top(self, margin_top: i32) -> Self {
                            Self { builder: self.builder.property("margin-top", margin_top), }
                        }

                            pub fn name(self, name: impl Into<glib::GString>) -> Self {
                            Self { builder: self.builder.property("name", name.into()), }
                        }

                            pub fn opacity(self, opacity: f64) -> Self {
                            Self { builder: self.builder.property("opacity", opacity), }
                        }

                            //pub fn overflow(self, overflow: /*Ignored*/gtk::Overflow) -> Self {
                        //    Self { builder: self.builder.property("overflow", overflow), }
                        //}

                            pub fn receives_default(self, receives_default: bool) -> Self {
                            Self { builder: self.builder.property("receives-default", receives_default), }
                        }

                            pub fn sensitive(self, sensitive: bool) -> Self {
                            Self { builder: self.builder.property("sensitive", sensitive), }
                        }

                            pub fn tooltip_markup(self, tooltip_markup: impl Into<glib::GString>) -> Self {
                            Self { builder: self.builder.property("tooltip-markup", tooltip_markup.into()), }
                        }

                            pub fn tooltip_text(self, tooltip_text: impl Into<glib::GString>) -> Self {
                            Self { builder: self.builder.property("tooltip-text", tooltip_text.into()), }
                        }

                            pub fn valign(self, valign: gtk::Align) -> Self {
                            Self { builder: self.builder.property("valign", valign), }
                        }

                            pub fn vexpand(self, vexpand: bool) -> Self {
                            Self { builder: self.builder.property("vexpand", vexpand), }
                        }

                            pub fn vexpand_set(self, vexpand_set: bool) -> Self {
                            Self { builder: self.builder.property("vexpand-set", vexpand_set), }
                        }

                            pub fn visible(self, visible: bool) -> Self {
                            Self { builder: self.builder.property("visible", visible), }
                        }

                            pub fn width_request(self, width_request: i32) -> Self {
                            Self { builder: self.builder.property("width-request", width_request), }
                        }

                            //pub fn accessible_role(self, accessible_role: /*Ignored*/gtk::AccessibleRole) -> Self {
                        //    Self { builder: self.builder.property("accessible-role", accessible_role), }
                        //}

                            pub fn orientation(self, orientation: gtk::Orientation) -> Self {
                            Self { builder: self.builder.property("orientation", orientation), }
                        }

    // rustdoc-stripper-ignore-next
    /// Build the [`Dropdown`].
    #[must_use = "Building the object from the builder is usually expensive and is not expected to have side effects"]
    pub fn build(self) -> Dropdown {
    self.builder.build() }
}
